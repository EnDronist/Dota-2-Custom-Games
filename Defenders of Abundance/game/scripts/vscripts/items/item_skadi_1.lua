...
_G.item_skadi_cold_duration_melee = keys.ability:GetSpecialValueFor("skadi_cold_duration_melee")
_G.item_skadi_cold_duration_ranged = keys.ability:GetSpecialValueFor("skadi_cold_duration_ranged")
if not keys.caster:HasModifier("modifier_item_desolator_thinker") then
	local desolator = 0
	local skadi = 0
	local maelstrom = 0
...
--If item loses
function modifier_item_skadi_lose(keys)
	local tick = 0.03
	Timers:CreateTimer(tick, function()
		...
		for i=0, 5, 1 do
			...
			if current_item ~= nil then
				if current_item:GetName() == "item_skadi_1" then a = 1 end
			end
		end
		if a == 0 then
			keys.caster:RemoveModifierByName("modifier_item_skadi_thinker")
			keys.caster:RemoveModifierByName("modifier_item_skadi_orb")
		end
	end)
end